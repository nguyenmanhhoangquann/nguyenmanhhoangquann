<div align="center">

<!-- HEADER TITLE -->
<h1 align="center">Hi there 👋, I'm Hoang Quan</h1>
<h3 align="center">🚀 Web Developer | Software Developer</h3>

<br/><br/>

<!-- ANIMATED BANNER -->
<img src="https://raw.githubusercontent.com/nguyenmanhhoangquann/nguyenmanhhoangquann/main/dev-banner.gif" width="100%"/>

<br/>

<!-- SOCIAL BADGES -->
[![Telegram](https://img.shields.io/badge/Telegram-2AABEE?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/hoangquan)
[![Facebook](https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white)](https://www.facebook.com/hoangquan12012006)
[![Gmail](https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:nguyenmanhhoangquan.vn@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/nguyenmanhhoangquann)

<br/><br/>

<!-- TERMINAL TYPING EFFECT -->
<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=500&size=17&pause=1000&color=A855F7&center=true&vCenter=true&width=550&lines=console.log(%22Hello%2C%20World!%20I'm%20Quan%22)%3B;Passionate+about+Full-Stack+Web+Development;Learning+Java+Spring+Boot+%26+Modern+React;Constantly+building%2C+debugging+%26+growing." alt="Typing SVG" />
</a>

<br/>

</div>

---

### 👨‍💻 Giới thiệu đôi nét
- 🎓 Sinh viên ngành **Công nghệ Thông tin** tại **Đại học Thăng Long (Hà Nội)**.
- 💻 Đam mê xây dựng các ứng dụng web thực tế: **Frontend (React)** và **Backend (Java Spring Boot)**.
- 🎯 Mục tiêu hiện tại: Tìm kiếm vị trí **Intern / Thực tập sinh** để cọ xát và học hỏi quy trình làm việc thực tế trong doanh nghiệp.
- 🚀 Sở thích: Tìm hiểu kiến trúc phần mềm, RESTful API và khám phá công nghệ web.

---

### 🛠 Công nghệ & Công cụ đang sử dụng

<div align="center">
  </div>

<br/>

---

### 📂 Dự án nổi bật

| Tên dự án | Công nghệ sử dụng | Điểm nổi bật | Mã nguồn |
|:---|:---|:---|:---:|
| 🛒 **Vinamilk E-commerce Web** | React, Spring Boot, MySQL, JWT | Web bán hàng đầy đủ tính năng: phân quyền, đặt hàng, hủy đơn & hoàn kho tự động | [Xem trên GitHub ↗](https://github.com/nguyenmanhhoangquann/vinamilk-web) |
| 📄 **ATS Resume Analyzer** | React, Spring Boot, Gemini API, PDFBox | Công cụ tải PDF và đánh giá CV tự động theo mô tả công việc bằng AI | [Xem trên GitHub ↗](https://github.com/nguyenmanhhoangquann/ATS-Resume-Analyzer) |

---

<div align="center">

<!-- FOOTER SLICE -->
<img src="https://capsule-render.vercel.app/api?type=slice&color=0:4A00E0,100:8E2DE2&height=90&section=footer" width="100%"/>

</div>
